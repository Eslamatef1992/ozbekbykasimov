...
export default function Cart() {
  const { items, subtotal, discount, coupon, couponError, applyCoupon, clearCoupon, updateItem, removeItem } = useCart();
  ...
  async function handleApplyCoupon(e) {
    ...
    try {
      ...
      // error message surfaced via couponError from context
    } finally {
      setApplying(false);
    }
  }
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start">
                ...
                return (
                  <div key={i.id} className="relative flex items-center gap-4 bg-mint/30 rounded-xl p-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-base text-ink/70 mb-1">{(Number(i.price) + lineExtras).toFixed(2)} Kd</div>
                      ...
                      <div className="flex items-center border border-ink/15 rounded-full w-fit overflow-hidden mt-2">
          ...
          <aside className="space-y-6">
            <div className="bg-white border border-border rounded-2xl p-6">
              ...
              {coupon ? (
                <div className="flex items-center justify-between bg-mint/40 rounded-lg px-4 py-3">
                  <span className="text-sm font-medium text-forest">{coupon.code} {t('applied') || 'applied'}</span>
                ...
                <form onSubmit={handleApplyCoupon} className="space-y-3">
                  ...
                  <button disabled={applying} className="w-full bg-forest text-white rounded-lg py-3 font-medium hover:bg-forest-dark transition disabled:opacity-50">
                    {applying ? '...' : t('apply')}
                  </button>
                </form>
              )}
            ...
            <div className="bg-white border border-border rounded-2xl p-6">
              ...
              <div className="space-y-2.5 text-base">
                <div className="flex justify-between"><span className="text-ink/60">{t('subtotal')}</span><span>{subtotal.toFixed(2)} Kd</span></div>
